...
template <class Typelist,
          unsigned int I,
          class Union>
bool get_union_caseI(const DDS_DynamicData &instance, 
                     const MemberAccess & ma, 
...
template <class Typelist,
          unsigned int I,
          class Union>
bool set_union_caseI(DDS_DynamicData &instance, 
                     const MemberAccess & ma, 
                     const Union& val)
{
     ...
     (I == val.get_variant().which()-1))
  {
    ...
    else
    {
      ...
      set_memberI_value(instance, MemberAccess::BY_NAME(info.name.c_str()), *data_ptr);  
    }
...
{
  typedef TypelistIterator<Typelist, 
                           I+1, 
                           MAX_INDEX> Next;
  ...
  static void set_union(DDS_DynamicData &instance, 
                        ...
                        const Union& val)
  {
    if(!set_union_caseI<Typelist, I>(instance, ma, val))
  ...
  static void get_union(const DDS_DynamicData &instance, 
                        ...
                        Union& val)
  {
    if(!get_union_caseI<Typelist, I>(instance, ma, discriminator_value, val))
...
struct TypelistIterator<Typelist, MAX_INDEX, MAX_INDEX>
{
  ...
  static void set_union(DDS_DynamicData &instance, 
                        ...
                        const Union& val)
  {
    if(!set_union_caseI<Typelist, MAX_INDEX>(instance, ma, val))
    {
      std::cerr << "Union was empty!!\n";
      throw 0;
    }
  }
  ...
  static void get_union(const DDS_DynamicData &instance, 
                        ...
                        Union& val)
  {
    if(!get_union_caseI<Typelist, MAX_INDEX>(instance, ma, discriminator_value, val))
    {
      std::cerr << "No match for the discriminator!!! (Did you forget to set a union member?)"
                << " discriminator_value = " 
                << discriminator_value << std::endl;
      throw 0;
    }
  }
};
