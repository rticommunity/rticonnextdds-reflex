...
EXPIMP_TEMPLATE template class DllExport SafeTypeCode<DDS_TypeCode>;
...
template <class T>
class GenericDataWriter : public DataWriterBase
{
public:
...
{
  typedef typename remove_refs<std::tuple<Args...>>::type NoRefsTuple;
...
template <class T>
class GenericDataReader : public DataReaderBase
{
public:
  ...
  DDS_ReturnCode_t take(std::vector<Sample<T>> & data,
