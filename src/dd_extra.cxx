...
void set_memberI_value(DDS_DynamicData & instance,                               \
  ...
  if (rc != DDS_RETCODE_OK) {                                                    \
    std::cerr << #FUNCTION " error = " << get_readable_retcode(rc) << "\n";      \
    throw 0;                                                                     \
  }                                                                              \
}
  ...
  if(ma.access_by_id())                                                            \
    ...
    rc = instance.FUNCTION(out, ma.get_name(),                                     \
                           DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED);                \
                                                                                   \
  if (rc != DDS_RETCODE_OK) {                                                      \
    std::cerr << #FUNCTION " error" <<  rc << std::endl;                           \
    throw 0;                                                                       \
  }                                                                                \
  val = out;                                                                       \
}
...
GET_VALUE_DEF(DDS_Boolean,          bool,              get_boolean)
                       ...
                       const std::string & val)
{
  ...
  if (rc != DDS_RETCODE_OK) {
    std::cerr << "set_string error = " << rc << std::endl;
    throw 0;
  }
}
...
{
#ifdef RTI_WIN32  
  static char * ptr = new char[MAX_STRING_SIZE+1];
#else
  ...
  DDS_ReturnCode_t rc = 
    instance.get_string(ptr, &size, member_name, id);
  ...
  if(rc != DDS_RETCODE_OK) 
  {
    ...
    if(rc != DDS_RETCODE_OK) {
      std::cout << "Unable to read string\n";
      throw 0;
    }
    val.assign(ptr);
    DDS_String_free(ptr);
    ...
    inner_(&inner)
{
  ...
  if(DDS_RETCODE_OK != rc) {
    std::cerr << "SafeBinder::bind_complex_member error=" 
